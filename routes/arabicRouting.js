...
router.get("/", (req, res) => {
  butter.page.retrieve('*','homepage-ar')
  .then(function(resp){
...
router.get(
  ...
  function (req, res) {
    butter.page.retrieve('*','accounttypes-ar')
    .then(function(resp){
      var page1=resp.data.data;
...
router.get(
  ...
  function (req, res) {
    butter.page.retrieve('*','documentspolicies-ar')
    .then(function(resp){
      var page1=resp.data.data;
...
router.get("/"+encodeURIComponent("ابدا-التداول") +
"/" +
encodeURIComponent("الإيداع-و-السحب"),(req,res)=>(
  butter.page.retrieve('*','depositandwithdrawal-ar')
  .then(function(resp){
    var page1=resp.data.data;
...
router.get("/"+encodeURIComponent("ابدا-التداول") +
"/" +
encodeURIComponent("رسوم-ايفست"),(req,res)=>(
  butter.page.retrieve('*','tradingfees-ar')
  .then(function(resp){
    var page1=resp.data.data;
...
router.get("/"+encodeURIComponent("ابدا-التداول") +
"/" +
encodeURIComponent("اوقات-التداول"),(req,res)=>(
  butter.page.retrieve('*','tradinghours-ar')
  .then(function(resp){
    var page1=resp.data.data;
...
router.get("/"+encodeURIComponent("ابدا-التداول") +
"/" +
encodeURIComponent("حاسبات-التداول"),(req,res)=>(
  butter.page.retrieve('*','tradingcalculator-ar')
  .then(function(resp){
    var page1=resp.data.data;
...
router.get("/"+encodeURIComponent("ابدا-التداول") +
"/" +
encodeURIComponent("تداول-الاسهم-0-عمولة"),(req,res)=>(
  butter.page.retrieve('*','zerocommission-ar')
  .then(function(resp){
    var page1=resp.data.data;
...
router.get("/"+encodeURIComponent("ابدا-التداول") +
"/" +
encodeURIComponent("حساب-تداول-تجريبي"),(req,res)=>(
  butter.page.retrieve('*','demoaccount-ar')
  .then(function(resp){
    var page1=resp.data.data;
...
router.get("/"+encodeURIComponent("ابدا-التداول") +
"/" +
encodeURIComponent("حساب-تداول-إسلامي"),(req,res)=>(
  butter.page.retrieve('*','islamic-ar')
  .then(function(resp){
    var page1=resp.data.data;
...
router.get("/"+encodeURIComponent("التداول-من-الجوال"),(req,res)=>{
  butter.page.retrieve('*','mobile-ar')
  .then(function(resp){
    var page1=resp.data.data;
...
router.get("/"+encodeURIComponent("ميتاتريدر-5"),(req,res)=>{
  butter.page.retrieve('*','metatrader-ar')
  .then(function(resp){
    var page1=resp.data.data;
...
router.get("/"+encodeURIComponent("تطبيق-تداول-ايفست"),(req,res)=>{
  butter.page.retrieve('*','tradingapp-ar')
  .then(function(resp){
    var page1=resp.data.data;
...
router.get("/"+encodeURIComponent("إيڤست")+"/"+encodeURIComponent("لماذا-إيڤست"),(req,res)=>{
  butter.page.retrieve('*','whyevest-ar')
  .then(function(resp){
    var page1=resp.data.data;
...
router.get("/"+encodeURIComponent("إيڤست")+"/"+encodeURIComponent("اتصل-بنا"),(req,res)=>{
  butter.page.retrieve('*','contactus-ar')
  .then(function(resp){
    var page1=resp.data.data;
...
router.post('/sendArabic',(req,res)=>{
  ...
  sgMail
    ...
    .then(function(resp){
      var page1=resp.data.data;
      res.render('ar/Evest/contactUs',{page:page1,message:'تم ارسال الرسالة بنجاح , سوف نقوم بتواصل معك باقرب وقت'});
    })

})

router.get("/"+encodeURIComponent("إيڤست")+"/"+encodeURIComponent("وظائف-إيڤست"),(req,res)=>{
  butter.page.retrieve('*','career-ar')
  .then(function(resp){
    var page1=resp.data.data;
...
router.get("/"+encodeURIComponent("إيڤست")+"/"+encodeURIComponent("مركز-المساعدة"),(req,res)=>{
  butter.page.retrieve('*','helpdesk-ar')
  .then(function(resp){
    var page1=resp.data.data;
...
router.get("/"+encodeURIComponent("إيڤست")+"/"+encodeURIComponent("تراخيص-التداول"),(req,res)=>{
  butter.page.retrieve('*','regulatoryauthorisation-ar')
  .then(function(resp){
    var page1=resp.data.data;
...
router.get("/"+encodeURIComponent("اخبار-النفط"),(req,res)=>{
  var page={fields:{seo:{meta_description:`أخبار النفط : هل أنت مهتم بأخبار النفط في الأسواق الخليجية والعالمية ؟ تبحث عن مواضيع حول النفط عبر الانترنت تابع تداول النفط عبر EVEST استثمر في الأسهم`,
  meta_keyword:` أخبار النفط`,page_title:`أخبار النفط - Evest | استثمر في الأسهم العالمية 100% بدون عمولة`}}};
  res.render('ar/Education/oil',{page:page});
});
router.get("/"+encodeURIComponent("اخبار-النفط")+"/:slug", async(req,res)=>{
...
router.get("/"+encodeURIComponent("اخبار-الذهب"),(req,res)=>{
  var page={fields:{seo:{meta_description:`اخبار تداول الذهب : هل أنت مهتم باخبار الذهب في الأسواق الخليجية والعالمية ؟ تبحث عن مواضيع حول الذهب عبر الانترنت تابع تداول EVEST`,
  meta_keyword:`اخبار تداول الذهب`,page_title:`اخبار تداول الذهب - أخبار سوق الذهب، تداول اون لاين | إيڤست وسيط موثوق 100% Evest`}}};
  res.render('ar/Education/gold',{page:page});
});
router.get("/"+encodeURIComponent("اخبار-الذهب")+"/:slug",async(req,res)=>{
  const url=`https://cms.evest.com/ar/wp-json/wp/v2/posts?_embed&slug=${req.params.slug}`;
  const options = {
    "method": "GET",
  }
...
router.get("/:slug"+"/"+encodeURIComponent("اخبار-الذهب"),async(req,res)=>{
  const url=`https://cms.evest.com/ar/wp-json/wp/v2/posts?_embed&slug=${req.params.slug}`;
  const options = {
    "method": "GET",
  }
...
router.get("/"+encodeURIComponent("أخبار-التداول"),(req,res)=>{
  var page={fields:{seo:{meta_description:`أخبار التداول : هل أنت مهتم بأخبار التداول في الأسواق الخليجية والعالمية ؟ تبحث عن مواضيع الأكثر ثقة عبر الانترنت, والمواضيع التي تهمك؟ تابع EVEST`,
  meta_keyword:`أخبار التداول`,page_title:`أخبار التداول ايفست | الأسواق المالية | استثمر في الأسهم العالمية 100% بدون عمولة Evest`}}};
  res.render('ar/Education/tradingNews',{page:page});
});
router.get("/"+encodeURIComponent("أخبار-التداول")+"/:slug",async(req,res)=>{
  const url=`https://cms.evest.com/ar/wp-json/wp/v2/posts?_embed&slug=${req.params.slug}`;
  const options = {
    "method": "GET",
  }
...
router.get("/:slug"+"/"+encodeURIComponent("أخبار-التداول"),async(req,res)=>{
  const url=`https://cms.evest.com/ar/wp-json/wp/v2/posts?_embed&slug=${req.params.slug}`;
  const options = {
    "method": "GET",
  }
...
router.get("/"+encodeURIComponent("اخبار-السوق"),(req,res)=>{
  var page={fields:{seo:{meta_description:`أخبار سوق التداول : هل أنت مهتم بأخبار سوق التداول في الأسواق الخليجية والعالمية ؟ تبحث عن مواضيع حول أخبار السوق والأكثر ثقة عبر الانترنت, تابعنا`,
  meta_keyword:`أخبار سوق التداول`,page_title:`أخبار سوق التداول - ايفست، تداول اون لاين | وسيط موثوق 100% Evest`}}};
  res.render('ar/Education/market',{page:page});
});
router.get("/"+encodeURIComponent("اخبار-السوق")+"/:slug",async(req,res)=>{
  const url=`https://cms.evest.com/ar/wp-json/wp/v2/posts?_embed&slug=${req.params.slug}`;
  const options = {
    "method": "GET",
  }
...
router.get("/:slug"+"/"+encodeURIComponent("اخبار-السوق"),async(req,res)=>{
  const url=`https://cms.evest.com/ar/wp-json/wp/v2/posts?_embed&slug=${req.params.slug}`;
  const options = {
    "method": "GET",
  }
...
router.get("/"+encodeURIComponent("مدونة-التداول"),(req,res)=>{
  var page={fields:{seo:{meta_description:`مدونة التداول : هل أنت مهتم باخر المواضيع التي تخص التداول اون لاين من أسهم ومؤشرات في الأسواق الخليجية والعالمية ؟ البحث عن مواضيع الأكثر ثقة عبر الانترنت, والمواضيع التي تهمك؟ تابع EVEST`,
  meta_keyword:`مدونة التداول`,page_title:`مدونة التداول - Evest ايفست، استثمر اون لاين مع إيفست EVEST`}}};
  res.render('ar/Education/blog',{page:page});
});

router.get("/"+encodeURIComponent("مدونة-التداول")+"/:slug",async(req,res)=>{
  const url=`https://cms.evest.com/ar/wp-json/wp/v2/posts?_embed&slug=${req.params.slug}`;
  const options = {
    "method": "GET",
  }
...
router.get("/:slug"+"/"+encodeURIComponent("مدونة-التداول"),async(req,res)=>{
  const url=`https://cms.evest.com/ar/wp-json/wp/v2/posts?_embed&slug=${req.params.slug}`;
  const options = {
    "method": "GET",
  }
