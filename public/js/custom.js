  ...
  if (check) {
    for (let i = 0; i < clicks.length; i++) {
      if (!clicks[i].className.includes("readmore")) {
        clicks[i].setAttribute(
          "onclick",
          "window.location.replace('/mobile/#/signup')"
        );
      }
    }
  }
...
function loginSuccessCallback(e) {
  ...
  for (let i = 0; i < clicks.length; i++) {
    if (
      clicks[i].className.includes("page") ||
      clicks[i].className.includes("readmore")
    ) {
      i++;
    } else {
      clicks[i].setAttribute(
        "onclick",
        "window.location.replace('/trade-room')"
      );
    }
  }
}
